...
static char *get_and_remove_option2(int *argc, char **argv, 
				    const char *option);
/* Forward declare AsyncIo typedef - defined below */
typedef struct async_io AsyncIo;

static int init_async_io(DriverData *dp, AsyncIo* aio, int use_threads);
static void release_async_io(AsyncIo* aio, ErlDrvPort);
static void async_read_file(AsyncIo* aio, LPVOID buf, DWORD numToRead);
static int async_write_file(AsyncIo* aio, LPVOID buf, DWORD numToWrite);
static int get_overlapped_result(AsyncIo* aio,
...
static int create_file_thread(AsyncIo* aio, int mode);
...
/* Windows cross-compilation compatibility: Split macro into getter and setter to avoid illegal lvalue cast */
#define OV_BUFFER_PTR(dp) ((LPVOID) ((dp)->ov.Internal))
#define OV_SET_BUFFER_PTR(dp, ptr) ((dp)->ov.Internal = (ULONG_PTR)(ptr))
