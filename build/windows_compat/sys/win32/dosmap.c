...
/* Only use for win32 if linking to MSVCR??.DLL and not if statically linking
   ...
   Windows cross-compilation: Enable for both dynamic and static builds with GNU toolchain.
   Original MSVC condition was for DLL builds only. */
...
*/
/* Windows cross-compilation: Remove __cdecl and leading underscore for GNU toolchain.
 * GNU calling convention doesn't add underscore prefix like MSVC does. */
void dosmaperr(unsigned long winerrno)
{
    _doserrno = winerrno;
