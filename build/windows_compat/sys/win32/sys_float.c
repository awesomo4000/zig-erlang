...
{
    /* Windows cross-compilation: Cast to avoid pointer signedness mismatch */
    unsigned char *s = (unsigned char *)buf, *t, *dp;
...
sys_double_to_chars_ext(double fp, char *buffer, size_t buffer_size, size_t decimals)
{
    ...
    while (*s) s++;
    /* Windows cross-compilation: Cast for pointer arithmetic with different signedness */
    return s - (unsigned char *)buffer; /* i.e strlen(buffer) */
}
