...
/* Windows cross-compilation: For static builds, provide the global
 * WinDynNifCallbacks that NIFs reference (declared extern in erl_nif.h).
 * In dynamic builds, each NIF DLL creates its own copy via ERL_NIF_INIT_GLOB. */
...
#undef ERL_NIF_API_FUNC_DECL
    ...
    /* Windows cross-compilation: For static builds, copy function pointers
     * to the global WinDynNifCallbacks that static NIFs reference. */
#ifdef STATIC_ERLANG_NIF
    memcpy(&WinDynNifCallbacks, &nif_callbacks, sizeof(TWinDynNifCallbacks));
#endif
