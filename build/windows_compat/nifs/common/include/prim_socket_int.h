...
/* Windows cross-compilation: Define struct cmsghdr for Windows */
#ifdef __WIN32__
#ifndef _STRUCT_CMSGHDR_DEFINED
#define _STRUCT_CMSGHDR_DEFINED
/* WSACMSGHDR is the Windows equivalent of struct cmsghdr
 * Define struct cmsghdr to match layout of WSACMSGHDR (struct _WSACMSGHDR)
 * Note: winsock2.h should already be included by files that use sockets */
struct cmsghdr {
    SIZE_T cmsg_len;
    INT    cmsg_level;
    INT    cmsg_type;
