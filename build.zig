...
pub fn build(b: *std.Build) void {
    ...
    const gen_step = generateSources(b, target, enable_jit);
    ...
    const ryu = buildRyu(b, target, optimize);
    const pcre = buildPcre(b, target, optimize);
    const ethread = buildEthread(b, target, optimize);
    const asmjit = if (enable_jit) buildAsmjit(b, target, optimize) else null;
...
fn generateSources(b: *std.Build, target: std.Build.ResolvedTarget, enable_jit: bool) *std.Build.Step {
